 ...
 */
async function cadastrarPergunta(pool: Pool) {
  ...
  try {
    ...
    const resPergunta = await client.query(
      'INSERT INTO "perguntas" (enunciado) VALUES ($1) RETURNING id_pergunta',
      [enunciado]
    );
    ...
    await client.query(
      'INSERT INTO "respostas" (id_pergunta, resposta_correta) VALUES ($1, $2)',
      [idPergunta, respostaCorreta]
    );
 ...
 */
async function jogarQuiz(pool: Pool) {
  let client: PoolClient | undefined;
  console.log('\n--- Iniciando o Quiz ---');
  ...
  try {
    ...
    for (const pergunta of perguntas) {
      ...
      const respostaResult = await client.query(
        'SELECT resposta_correta FROM "respostas" WHERE id_pergunta = $1',
    ...
    await client.query(
        'INSERT INTO "pontos" (id_usuario, pontos) VALUES ($1, $2)',
        [idUsuario, pontos]
    );
